                                ...
                                <div class="mt-3">
                                    <form method="POST" action="/admin/orders/<?= $order['id'] ?>/status" class="status-form inline-block" data-order-id="<?= $order['id'] ?>">
                                        ...
                                        <select name="delivery_status" class="status-select text-sm border border-gray-300 rounded px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 <?php
                                            $currentStatus = $order['delivery_status'] ?? 'En cours de préparation';
                                            if (!mb_check_encoding($currentStatus, 'UTF-8')) {
                                                $currentStatus = mb_convert_encoding($currentStatus, 'UTF-8', 'ISO-8859-1');
                                            }
                                            ...
                                            echo $statusColor;
                                        ?>">
                                            <option value="En cours de préparation" <?= $currentStatus === 'En cours de préparation' ? 'selected' : '' ?>>En cours de préparation</option>
                                            <option value="Expédiée" <?= $currentStatus === 'Expédiée' ? 'selected' : '' ?>>Expédiée</option>
